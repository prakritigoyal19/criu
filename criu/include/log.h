...
#ifndef LOG_BINARY
	#define recieve(loglevel, fmt, ...)		print_on_level(loglevel, fmt, ##__VA_ARGS__);
#else
	#define recieve(loglevel, fmt, ...)		flog_encode(loglevel, fmt, ##__VA_ARGS__);
#endif
...
#define print_once(loglevel, fmt, ...)					\
	do {								\
		...
		if (!__printed) {					\
			recieve(loglevel, fmt, ##__VA_ARGS__);	\
			__printed = 1;					\
		}							\
	} while (0)
